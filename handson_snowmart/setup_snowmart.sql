...
-- GitHubから自動でデータ取得します。CSVの手動アップロードは不要です。
--
-- 【処理内容】
-- Step 1: 環境設定
...
-- Step 9: ノートブックのオープン手順（Workspace）
-- Step 10: 全オブジェクト確認
--
...
-- Step 9: ノートブックのオープン手順（Workspace）
-- ============================================================
-- Workspace Notebook はGitリポジトリから直接開きます。
-- 以下の手順で参加者各自がノートブックを開いてください。
--
-- 【手順】
-- 1. Snowsight 左メニュー → Projects → Notebooks
-- 2. 右上「+ Notebook」ドロップダウン →「Import from Repository」
-- 3. Repository: snowmart_handson_repo
--    Branch: main
--    Path: handson_snowmart/snowmart_ai_handson.ipynb
-- 4. 「Import」をクリック → ノートブックが開きます
--
-- ※ Git Repository 統合（snowmart_handson_repo）は Step 3 で作成済みです。

SELECT '【Step 9】上記手順でWorkspace Notebookを開いてください' AS STATUS;
...
-- [ ] STORE_DOCUMENTS       : 10行
-- [ ] Workspace Notebook    : snowmart_ai_handson.ipynb をGitリポジトリからインポート済み
--
-- セッション中に参加者が作成するオブジェクト（このSQLには含めない）:
-- [ ] SNOWMART_DOC_SEARCH   : Cortex Search Service（Scene 2 で作成）
